<template>
  ...
  <div class="xcTable" :style="{'height':_height}" ref="xcTableMain" v-loading="loading">
		<div class="xcTable-table" :style="{'height':_table_height}">
			<el-table v-bind="$attrs" :data="tableData" :row-key="rowKey" :key="toggleIndex" ref="xcTable" :height="height=='auto'?null:'100%'" :size="config.size" :border="config.border" :stripe="config.stripe" :summary-method="remoteSummary?remoteSummaryMethod:summaryMethod" @sort-change="sortChange" @filter-change="filterChange"
		...
		<div class="xcTable-page" v-if="!hidePagination || !hideDo">
			<div class="xcTable-pagination">
			...
			<div class="xcTable-do" v-if="!hideDo">
	...
	export default {
		...
		methods: {
			...
			async getData(){
				...
				this.$refs.xcTable.setScrollTop(0)
			...
			async getApiData(){
				...
				try {
					const url= `${apiConfig.API_URL}`+this.url;
					var res = await http.post(url, reqData, {
						headers: {
							//'response-status': 401
						}
					});
			...
			//刷新数据
			refresh(){
				this.$refs.xcTable.clearSelection();
				if(this.apiObj){
					this.getData();
				}else if(this.url){
					this.getApiData();
			...
			reload(params, page=1){
				...
				this.$refs.xcTable.clearSelection();
				this.$refs.xcTable.clearSort()
				this.$refs.xcTable.clearFilter()
				if(this.apiObj){
					this.getData();
				}else if(this.url){
					this.getApiData();
			...
			toggleRowSelection(row, selected){
				this.$refs.xcTable.toggleRowSelection(row, selected)
			},
			toggleAllSelection(){
				this.$refs.xcTable.toggleAllSelection()
			},
			toggleRowExpansion(row, expanded){
				this.$refs.xcTable.toggleRowExpansion(row, expanded)
			},
			setCurrentRow(row){
				this.$refs.xcTable.setCurrentRow(row)
			},
			clearSort(){
				this.$refs.xcTable.clearSort()
			},
			clearFilter(columnKey){
				this.$refs.xcTable.clearFilter(columnKey)
			},
			doLayout(){
				this.$refs.xcTable.doLayout()
			},
			sort(prop, order){
				this.$refs.xcTable.sort(prop, order)
			},
			/**
			 * 获取选中行的 ID 列表
			 * @returns {Array} 选中行的 ID 列表
			 */
			getSelectedIds() {
...
<style scoped>
	.xcTable {}
	.xcTable-table {height: calc(100% - 50px);}
	.xcTable-page {height:50px;display: flex;align-items: center;justify-content: space-between;padding:0 15px;}
	.xcTable-do {white-space: nowrap;}
	.xcTable:deep(.el-table__footer) .cell {font-weight: bold;}
	.xcTable:deep(.el-table__body-wrapper) .el-scrollbar__bar.is-horizontal {height: 12px;border-radius: 12px;}
	.xcTable:deep(.el-table__body-wrapper) .el-scrollbar__bar.is-vertical {width: 12px;border-radius: 12px;}
</style>
